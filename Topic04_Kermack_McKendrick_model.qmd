...
Suppose that infected individuals exert an average force of infection $f(\tau)$ where $\tau$ is the age of infection. $f(\tau)$ is equivalent to the average number of transmissions per unit time from a single infective with age of infection $\tau$ in a fully susceptible population, and we will refer to it as the *infectiousness*.  
...
The SIR model is a special case of the Kermack-McKendrick model with $f(\tau)=\beta e^{-\gamma \tau}$. To see this, note that this gives 
$$
\lambda(t)= \frac{\beta}{N} \int_0^\infty \inc(t-\tau) e^{-\gamma \tau} d\tau
$$
A Laplace transform of the equation for $\dot{I}$ in the SIR model shows that this is equivalent to $\lambda(t) = \beta I(t)/N$, which is the correct force of infection for the SIR model. 
...
$$
f_\mathrm{iso}(\tau) = \left\{\begin{array}{ll} f(\tau), & 0\le\tau<t_\mathrm{iso} \\   c_\mathrm{iso} (1-p_\mathrm{iso}) f(\tau),&  \tau\ge t_\mathrm{iso} \end{array}  \right.
$$
...
$$
\begin{align}
\R_c &= \R_0 \left[ \int_0^{t_\mathrm{iso}} g(\tau) d\tau + c_\mathrm{iso} (1-p_\mathrm{iso})\int_{t_\mathrm{iso}}^\infty g(\tau) d\tau \right] \\
&= \R_0 \left[ F(t_\mathrm{iso}) + c_\mathrm{iso} (1-p_\mathrm{iso}) \left(1-F(t_\mathrm{iso})\right)  \right]
\end{align}
$$
